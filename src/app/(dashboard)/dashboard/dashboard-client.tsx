...
export default function DashboardClient({ initialSales, initialProducts }: DashboardClientProps) {
  const { user } = useAuth();
  ...
  const topProducts = useMemo(() => {
    ...
    filteredSales.forEach(sale => {
      (sale.items || []).forEach(item => {
        ...
        productMap[p.id].quantity += item.quantity || 0;
        productMap[p.id].totalRevenue += (item.quantity || 0) * (item.price || 0);
      });
    });
  ...
  const handleReplenishStock = async (productId: string, size: string, amount: number) => {
  ...
  const getChartSubtitle = () => {
    switch (filterType) {
      ...
      default:
        return 'Detalhamento de receita do caixa.';
    }
  };
  ...
  const handleExportXlsx = () => {
    ...
    const dataToExport = filteredSales.map((sale) => {
      const productsStr = (sale.items || [])
        .map((item) => `${item.quantity || 0}x ${item.product?.name || 'Desconhecido'} (${item.product?.brand || ''}${item.product?.colorway ? ` - ${item.product.colorway}` : ''} - Tam. ${item.size || ''})`)
        .join(', ');
    ...
    <div className="flex-1 bg-zinc-50 dark:bg-[#09090b] p-4 lg:p-8 font-sans antialiased">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        ...
        <div className="bg-white dark:bg-zinc-950 border border-zinc-200/50 dark:border-zinc-900 shadow-sm p-5 rounded-3xl relative overflow-hidden select-none flex flex-col justify-between h-36">
          <div className="flex items-center justify-between">
            <span className="text-[10px] text-zinc-400 dark:text-zinc-550 uppercase font-bold tracking-wider">Faturamento</span>
            <div className="p-2 rounded-xl bg-emerald-50 dark:bg-emerald-950/20 text-emerald-550 dark:text-emerald-450 border border-emerald-100 dark:border-emerald-900/30">
              <DollarSign className="w-4.5 h-4.5" />
            </div>
          </div>
          <div>
            <h3 className="text-2xl font-black tracking-tight text-zinc-950 dark:text-zinc-50 mt-2 leading-none">
        ...
        <div className="bg-white dark:bg-zinc-950 border border-zinc-200/50 dark:border-zinc-900 shadow-sm p-5 rounded-3xl relative overflow-hidden select-none flex flex-col justify-between h-36">
          <div className="flex items-center justify-between">
            ...
            <div className="p-2 rounded-xl bg-blue-50 dark:bg-blue-950/20 text-blue-550 dark:text-blue-450 border border-blue-100 dark:border-blue-900/30">
        ...
        <div className="bg-white dark:bg-zinc-950 border border-zinc-200/50 dark:border-zinc-900 shadow-sm p-5 rounded-3xl relative overflow-hidden select-none flex flex-col justify-between h-36">
          <div className="flex items-center justify-between">
            ...
            <div className="p-2 rounded-xl bg-purple-50 dark:bg-purple-950/20 text-purple-550 dark:text-purple-450 border border-purple-100 dark:border-purple-900/30">
        ...
        <div className="bg-white dark:bg-zinc-950 border border-zinc-200/50 dark:border-zinc-900 shadow-sm p-5 rounded-3xl relative overflow-hidden select-none flex flex-col justify-between h-36">
          <div className="flex items-center justify-between">
            ...
            <div className="p-2 rounded-xl bg-amber-50 dark:bg-amber-955/20 text-amber-550 dark:text-amber-450 border border-amber-100 dark:border-amber-900/30">
              <Target className="w-4.5 h-4.5" />
            </div>
          </div>
          <div>
            <div className="flex items-baseline justify-between mt-2">
              <h3 className="text-2xl font-black tracking-tight text-zinc-950 dark:text-zinc-50 leading-none">
            ...
            <div className="w-full bg-zinc-100 dark:bg-zinc-900 h-1.5 rounded-full mt-2.5 overflow-hidden">
              <div 
                className="h-full bg-gradient-to-r from-amber-400 to-amber-500 rounded-full transition-all duration-550" 
            ...
            <p className="text-[9px] text-zinc-400 dark:text-zinc-500 mt-2 flex items-center gap-1">
              {todayRevenue >= dailyGoal ? (
                ...
                <span>
                  Faltam <span className="font-semibold text-zinc-700 dark:text-zinc-350">{formatCurrency(dailyGoal - todayRevenue)}</span> para a meta
                </span>
              )}
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200/50 dark:border-zinc-900 rounded-3xl p-6 shadow-sm flex flex-col justify-between min-h-[350px]">
                  ...
                  {chartData.map((d, index) => {
                    ...
                    return (
                      <div key={index} className="flex-1 flex flex-col items-center group relative h-full justify-end">
                        ...
                        {/* Day label */}
                        <span className="text-[9px] text-zinc-400 dark:text-zinc-550 mt-2 font-medium truncate max-w-full text-center">
                          {d.dayName}
                        </span>
                      </div>
                    );
            ...
            <div className="border-t border-zinc-150 dark:border-zinc-900/60 pt-4 mt-4 flex items-center justify-between text-[10px] text-zinc-400">
          ...
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200/50 dark:border-zinc-900 rounded-3xl p-6 shadow-sm flex flex-col justify-between min-h-[350px]">
            <div className="flex-1 flex flex-col justify-between">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xs font-bold text-zinc-905 dark:text-zinc-150 uppercase tracking-wider">Transações do Período</h3>
              ...
              {filteredSales.length === 0 ? (
                ...
                <div className="space-y-3.5 max-h-60 overflow-y-auto pr-1">
                  {filteredSales.slice(0, 5).map((sale) => (
                    <div 
                      key={sale.id}
                      className="flex items-center justify-between p-2.5 hover:bg-zinc-50/50 dark:hover:bg-zinc-900/60 rounded-xl transition-colors border border-transparent hover:border-zinc-200/10"
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        <div className="w-7.5 h-7.5 rounded-lg bg-zinc-100 dark:bg-zinc-900 flex items-center justify-center shrink-0 border border-zinc-200/20 text-zinc-550">
            ...
            <div className="border-t border-zinc-150 dark:border-zinc-900/60 pt-4 mt-4 flex justify-end">
              <button
                onClick={() => router.push('/historico')}
                className="text-[9px] text-zinc-400 font-bold bg-transparent border-0 flex items-center gap-0.5 cursor-pointer hover:text-zinc-600 dark:hover:text-zinc-200 transition-colors focus:outline-none"
              >
                  ...
                  {topProducts.map((p, idx) => (
                    <div key={p.id} className="flex items-center justify-between gap-3 p-1.5 hover:bg-zinc-50 dark:hover:bg-zinc-900/40 rounded-2xl transition-colors">
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        {/* Info */}
                        <div className="min-w-0">
                          <p className="text-[11px] font-bold text-zinc-805 dark:text-zinc-200 truncate leading-tight">
                            {p.name}
                          </p>
            ...
            <div>
              ...
              {stockAlerts.length === 0 ? (
                <div className="py-8 text-center text-zinc-400">
                  <p className="text-xs font-semibold">Tudo em ordem!</p>
                  <p className="text-[10px] opacity-75 mt-0.5">Não há alertas de estoque crítico no momento.</p>
                </div>
              ) : (
                <div className="space-y-3 max-h-[280px] overflow-y-auto pr-1">
                  {stockAlerts.map((alert) => (
                    <div key={alert.id} className="flex items-center justify-between gap-3 p-1.5 hover:bg-zinc-55 dark:hover:bg-zinc-900/40 rounded-2xl transition-colors">
                      <div className="flex items-center gap-2.5 min-w-0">
                        {/* Product Thumbnail */}
                      ...
                      <div className="shrink-0">
                        {alert.stock === 0 ? (
                          <span className="inline-flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded-full bg-red-50 dark:bg-red-955/20 text-red-650 dark:text-red-400">
                            <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />
                            Esgotado
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded-full bg-yellow-50 dark:bg-yellow-955/20 text-yellow-600 dark:text-yellow-405">
      ...
      {isReplenishmentOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/45 backdrop-blur-md transition-all duration-300 animate-in fade-in">
          <div className="w-full max-w-xl overflow-hidden bg-white/95 dark:bg-zinc-905/95 border border-zinc-200/50 dark:border-zinc-800/50 shadow-2xl rounded-3xl p-6 md:p-8 flex flex-col max-h-[80vh] transform scale-100 transition-transform duration-300 animate-in zoom-in-95">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-1 mb-6">
              {stockAlerts.map((alert) => {
                ...
                return (
                  <div 
                    key={alert.id} 
                    className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 bg-zinc-50/50 dark:bg-zinc-900/30 border border-zinc-150/40 dark:border-zinc-900/40 rounded-2xl hover:border-zinc-250 dark:hover:border-zinc-850 transition-colors"
                  >
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[10px] text-zinc-455 dark:text-zinc-400 mt-1 leading-none">
                          Tamanho {alert.size} • {alert.brand}
                        </p>
                      </div>
                      ...
                      <div className="shrink-0 sm:mr-3">
                        {alert.stock === 0 ? (
                          <span className="inline-flex items-center gap-1 text-[8px] font-extrabold px-2 py-0.5 rounded-full bg-red-50 dark:bg-red-955/20 text-red-650 dark:text-red-400">
                            Esgotado
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 text-[8px] font-extrabold px-2 py-0.5 rounded-full bg-yellow-50 dark:bg-yellow-955/20 text-yellow-600 dark:text-yellow-405">
                            Restam {alert.stock}
                          </span>
                        )}
                      </div>
            ...
            <div className="flex justify-between items-center pt-4 border-t border-zinc-150/40 dark:border-zinc-900/60">
              <button
                ...
                className="py-2.5 px-4 text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-200 font-semibold hover:bg-zinc-50/50 dark:hover:bg-zinc-800/40 rounded-2xl transition-all text-xs cursor-pointer focus:outline-none"
              >
                Ver todo o estoque
              </button>
              <button
                onClick={() => setIsReplenishmentOpen(false)}
                className="py-2.5 px-5 bg-zinc-100 hover:bg-zinc-200/80 active:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:active:bg-zinc-800 text-zinc-800 dark:text-zinc-205 font-medium rounded-2xl transition-all text-xs cursor-pointer focus:outline-none"
              >
                Fechar Painel
              </button>
            </div>
